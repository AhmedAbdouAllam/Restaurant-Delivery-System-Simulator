...
GUI::GUI()
{
	...
	////
	OrdersClrs[TYPE_Politicians] = LIGHTGREEN;   //Politicians-order color
	OrdersClrs[TYPE_Disabled] = SALMON;         //Disabled-order color  
	////
	ClearStatusBar();
	ClearDrawingArea(); 
	DrawRestArea();  
...
void GUI::PrintStatBar(string msg,int x, int y) const	//Prints a message on status bar
{
//First clear the status bar

	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10+x, WindHeight - (int)(StatusBarHeight / 1.5)+y-30, msg); // You may need to change these coordinates later 
																		  // to be able to write multi-line
}

void GUI::PrintUpBar(string msg, int x, int y,color C) const
{
	//RestStartY
	if (y < RestStartY)
	{
		...
		pWind->DrawString(10 + x, y, msg); // You may need to change these coordinates later 
																			  // to be able to write multi-line
	}
}
